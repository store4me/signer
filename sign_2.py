...
def run():
    ...
    with tempfile.TemporaryDirectory() as temp_dir_str:
        ...
        print(f"Signed IPA created at {signed_ipa.resolve()}")


if __name__ == "__main__":
    # ---------------------------------------------------------------------
    # CLI argument handling (for GitHub Actions / signer.yml)
    # ---------------------------------------------------------------------
    parser = argparse.ArgumentParser(description="Offline re-signing helper (legacy sign_2.py)")
    parser.add_argument("--unsigned-ipa", default="unsigned.ipa", help="Path to input unsigned IPA")
    parser.add_argument("--cert", default="cert.p12", help="Path to .p12 certificate to use")
    parser.add_argument("--cert-pass", required=True, help="Password for the .p12 certificate")
    parser.add_argument("--team-id", required=True, help="Apple Team ID (10 chars)")
    parser.add_argument("--prov", default="prov.mobileprovision", help="Provisioning profile to embed")
    parser.add_argument("--bundle-id", default="", help="Override main CFBundleIdentifier (optional)")
    parser.add_argument("--bundle-name", help="Override CFBundleDisplayName (optional)")
    # flag equivalents of the historical sign_args switches
    parser.add_argument("-d", action="store_true", help="Enable debugging (get-task-allow)")
    parser.add_argument("-a", action="store_true", help="Patch to support all device families")
    parser.add_argument("-m", action="store_true", help="Enable Mac-style multi-window (iPad)")
    parser.add_argument("-s", action="store_true", help="Enable file-sharing / document browser")
    parser.add_argument("-e", action="store_true", help="Encode / randomise bundle IDs in entitlements")
    parser.add_argument("-p", action="store_true", help="Byte-patch binaries/resources with new IDs")
    parser.add_argument("-o", action="store_true", help="Force original CFBundleIdentifier to remain")

    cli = parser.parse_args()

    # ------------------------------------------------------------------
    # Create helper files expected by the legacy logic
    # ------------------------------------------------------------------
    from shutil import copy2
    # certificate + provisioning profile + ipa copied to canonical filenames if necessary
    cert_dest = Path("cert.p12")
    if Path(cli.cert).resolve() != cert_dest.resolve():
        copy2(cli.cert, cert_dest)

    prov_dest = Path("prov.mobileprovision")
    if cli.prov and Path(cli.prov).resolve() != prov_dest.resolve():
        copy2(cli.prov, prov_dest)

    ipa_dest = Path("unsigned.ipa")
    if Path(cli.unsigned_ipa).resolve() != ipa_dest.resolve():
        copy2(cli.unsigned_ipa, ipa_dest)

    # simple text helper files
    Path("cert_pass.txt").write_text(cli.cert_pass)
    Path("team_id.txt").write_text(cli.team_id)

    # build sign_args string matching original single-line format
    flag_map = {
        "-d": cli.d,
        "-a": cli.a,
        "-m": cli.m,
    ...
    sign_args_str = " ".join(k for k, v in flag_map.items() if v)
    Path("args.txt").write_text(sign_args_str)

    # optional bundle overrides
    Path("user_bundle_id.txt").write_text(cli.bundle_id)
    if cli.bundle_name is not None:
        Path("bundle_name.txt").write_text(cli.bundle_name)

    # ------------------------------------------------------------------
    # Continue with original file-driven workflow
    # ------------------------------------------------------------------
    ...
    if not unsigned_ipa.exists():
        raise Exception("unsigned.ipa not found in current directory.")
